...
VALID_COMPILERS = ("gcc", "clang", "cc", "llvm-ar")
...
class BitcodeCompilation(object):
    ...
    def extract(self, cmdline):
        ...
        d = {"flags": [], "output": None, "source": [], "tool": cmdline[0]}
    ...
    def make_compile_cmd(self, info, pwd):
        ...
        cmd.append("-g0 -O0")  # Debug info removal
        cmd += info["source"]
        cmd += ["-o", bc_name, "-c"]
    ...
    def make_link_cmd(self, info, pwd):
        ...
        remove_asm = []
        remove_asm = [
            e for e in info["source"] if e.replace(".o", "") not in "\n".join(self.asm)
        ]
        ...
        for v in sources:
            ...
            if not full_path in self.produced_bc:
                self.warn("File {} has not been produced by this run".format(full_path))
                continue
...
if __name__ == "__main__":
    ...
    gen = subparsers.add_parser(
        "generate", help="Generate a shell script that will build libraries as bitcode"
    )

    # Commands to provide which libraries were built
    dump = subparsers.add_parser(
        "dump", help="Produce a list of libraries that were built"
    )
    ...
    for bear_cmd in cmd_list:
        if bear_cmd["command"][0].split("/")[-1] not in VALID_COMPILERS:
            continue
